...
if(uGIMP==1)
    mpD.l = mpD.l0                                                        ;% half-length
elseif(cpGIMP==1)
%    mpD.l = repmat(mpD.J.^(meD.DoF),1,size(mpD.l,2)).*mpD.l0              ;% half-length
      mpD.dU= sqrt([(mpD.dF(:,1).^2+mpD.dF(:,3).^2),...
          (mpD.dF(:,2).^2+mpD.dF(:,4).^2)])                                 ;% diagonal components of the incremental stretch deformation, see Charlton etal, 2017
      mpD.l = mpD.dU.*mpD.l                                                 ;% -   
