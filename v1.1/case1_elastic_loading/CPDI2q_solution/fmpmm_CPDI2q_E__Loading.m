...
for sim=1:length(numel)
    ...
    [mpD]    = mpSetup(meD,ni,l0,coh0,cohr,phi0,phir,n0,rho0,nstr,typeD)  ;% - see function
    y0       = mpD.x(:,2);
    ...
    % PLOT SETTING
    [pp] = plotSetup(meD.xB(1),meD.xB(2),meD.xB(3),l0,meD.y,bc.y)         ;% - see function
    %---------------------------------------------------------------------%
    ...
    name=['.\data\CPDI2q_data_' num2str(sim) '.mat'];
