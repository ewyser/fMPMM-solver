...
mpD.dF(:,1) = 1.0+accumarray(mp,mpD.dSx.*meD.u(nX),[mpD.n 1])             ;% incremental deformation gradient tensor xx
mpD.dF(:,2) = 0.0+accumarray(mp,mpD.dSy.*meD.u(nX),[mpD.n 1])             ;% incremental deformation gradient tensor xy
mpD.dF(:,3) = 0.0+accumarray(mp,mpD.dSx.*meD.u(nY),[mpD.n 1])             ;% incremental deformation gradient tensor yx
mpD.dF(:,4) = 1.0+accumarray(mp,mpD.dSy.*meD.u(nY),[mpD.n 1])             ;% incremental deformation gradient tensor yy
%--------------------------------------------------------------------------%
...
mpD.r1(:,1) = mpD.dF(:,1).*mpD.r1(:,1)+mpD.dF(:,2).*mpD.r1(:,2)           ;% update domain vector 1
mpD.r1(:,2) = mpD.dF(:,3).*mpD.r1(:,1)+mpD.dF(:,4).*mpD.r1(:,2)           ;% update domain vector 1
mpD.r2(:,1) = mpD.dF(:,1).*mpD.r2(:,1)+mpD.dF(:,2).*mpD.r2(:,2)           ;% update domain vector 2
mpD.r2(:,2) = mpD.dF(:,3).*mpD.r2(:,1)+mpD.dF(:,4).*mpD.r2(:,2)           ;% update domain vector 3
