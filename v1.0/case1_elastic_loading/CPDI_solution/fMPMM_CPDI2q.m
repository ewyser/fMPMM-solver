...
for sim=1:length(numel)
    ...
    clear row                                                             ;%
    %---------------------------------------------------------------------%
    plot(meD.x,meD.y,'s',meD.x([BC.xi;BC.xs;BC.yi]),meD.y([BC.xi;BC.xs;BC.yi]),'gs'),axis equal
    %% MPM DISCRETIZATION
    [mpD]   = mpSetupElasticLoading(meD,ni,l0,xB(1),xB(2),yinf,coh0,cohr,phi0,phir,n0,rho0,nstr,typeD);% - see function
